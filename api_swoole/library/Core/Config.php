...
class Config {
     ...
     *
     * @access public
     * @param string $name 配置参数名（支持二级、三级配置 . 号分割）
     * @param mixed $default 默认值，配置项不存在时返回
     * @param string $range 作用域
     * @return mixed
     */
    public static function get($name = null, $default = null, $range = '') {
        $range = $range ?: self::$range;
        ...
        $levelNum = substr_count($name, '.') + 1;
        // 二维数组设置和获取支持
        $name = explode('.', $name, $levelNum);
        if ($levelNum == 2) { // 二级配置
            $name[0] = strtolower($name[0]);
    
            return isset(self::$config[$range][$name[0]][$name[1]]) ? self::$config[$range][$name[0]][$name[1]] : $default;
        } else if($levelNum == 3) { // 三级配置
            $name[0] = strtolower($name[0]);

            return isset(self::$config[$range][$name[0]][$name[1]][$name[2]]) ? self::$config[$range][$name[0]][$name[1]][$name[2]] : $default;
        }
