...
public class ${apiName?cap_first} extends AbstractService {
    ...
    public void check(${apiName?cap_first}Dto dto) {
        ...
        <#list dtoFields as field>
            <#if field.fieldUnique??>
        if (${tableName}Dao.cnt(${tableName?cap_first}Po.builder()
                <#if entityHasIsDelete>
             .whereIsDelete(IsDeleteEnum.NO_DELETE.getCode())
                </#if>
                <#list dtoFields as fieldWhere>
                    <#if fieldWhere.updateWhere?? == true>
            .whereNot${fieldWhere.fieldName?cap_first}(dto.get${fieldWhere.fieldName?cap_first}())
                    </#if>
                </#list>
    ...
    public Result<?> service(${apiName?cap_first}Dto dto) {
        if (${tableName}Dao.update(handlUpdateData(dto)) != 1) {
            throw new ServerException("更新失败");
        }

        <#if voHasUpdatedAt>
        return Result.ok("更新成功", Map.of("updatedAt", ${tableName}Dao.getFieldById(dto.getId(),  ${tableName?cap_first}::getUpdatedAt).orElse("")));
        <#else>
    ...
    * @return 处理后的数据
    */
    protected ${tableName?cap_first}Po handlUpdateData(${apiName?cap_first}Dto dto){
        ${tableName?cap_first}Po po = new ${tableName?cap_first}Po();
        <#list dtoFields as field>
            <#if field.fieldName != "updatedAt">
                <#if field.updateWhere?? == true>
        po.setWhere${field.fieldName?cap_first}(dto.get${field.fieldName?cap_first}());
                <#else>
        po.set${field.fieldName?cap_first}(dto.get${field.fieldName?cap_first}());
                </#if>
            </#if>
        </#list>
